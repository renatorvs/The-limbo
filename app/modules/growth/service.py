...
def get_campaigns(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Campaign).filter(models.Campaign.status == 'Active').offset(skip).limit(limit).all()
...
def get_latest_funnel(db: Session):
    return db.query(models.FunnelSnapshot).order_by(desc(models.FunnelSnapshot.date)).first()

def get_metrics(db: Session):
    # Fetch latest daily metrics
    latest_metrics = db.query(AnalyticsSaasMetricsDaily).order_by(desc(AnalyticsSaasMetricsDaily.reference_date)).first()
